...
``src/bitvavo_dca/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test bitvavo_dca
...
* ``DB_NAME``: name of the database for the project. Defaults to ``bitvavo_dca``.
* ``DB_USER``: username to connect to the database with. Defaults to ``bitvavo_dca``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``bitvavo_dca``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting bitvavo_dca_db_1 ... done
       Starting bitvavo_dca_web_1 ... done
   ...
   .. code-block:: bash
       ...
       $ docker logs -f bitvavo_dca_web_1
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it bitvavo_dca_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it bitvavo_dca_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/bitvavo_dca/
...
environment variables. See ``src/bitvavo_dca/conf/docker.py`` for
all settings.

.. code-block:: bash

    $ docker build -t bitvavo_dca
    $ docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    $ docker exec -it bitvavo_dca /app/src/manage.py createsuperuser
